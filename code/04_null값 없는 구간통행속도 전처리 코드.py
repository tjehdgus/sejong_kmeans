...
# 5. 원래 데이터와 시간대별 평균값 병합
merged_df = pd.merge(df2[['요일', '도로명', '구간', '시', '방향', '00분', '15분', '30분', '45분']],
                     day_time_mean_v[['요일', '도로명', '구간', '시', '방향', '00분', '15분', '30분', '45분']],
                     on=['요일', '도로명', '구간', '시', '방향'],
                     suffixes=('', '_평균'))

# 6. 평균과 두 번째로 큰 값을 병합
final_df = pd.merge(merged_df, second_largest_values, on=['요일', '도로명', '구간', '방향'], suffixes=('', '_최대값'))

# 7. 최대값과 평균값의 차이를 계산하는 새로운 컬럼 추가
...
final_result.to_csv("f:\\data\\세종시_구간통행속도_전처리.csv", index=False)
